...
interface CharacterGridSectionProps {
  ...
  isActiveLocationLocked: boolean;
  onClearFilter: () => void;
...
const CharacterGridSection: React.FC<CharacterGridSectionProps> = ({
  ...
  activeLocationId,
  activeLocationName,
  isActiveLocationLocked,
  onClearFilter,
  onSelectCharacter,
}) => {
  const showLockedEmpty = Boolean(activeLocationId && isActiveLocationLocked);
        ...
        {characters.length === 0 ? (
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
            {characters.map((char, index) => {
              const isRumorCard = !activeLocationId && char.discoveryStage === 'rumor';
              ...
              if (isRumorCard) {
                return (
                  <motion.div
                    ...
                    className="relative h-[400px] overflow-hidden bg-slate-900/70 border border-white/10 opacity-80"
                  >
                    <div className="absolute inset-0">
                      <img
                        src={char.imageUrl}
                        alt=""
                        className="w-full h-full object-cover grayscale blur-sm opacity-40"
                      />
                      <div className="absolute inset-0 bg-gradient-to-t from-slate-950/95 via-slate-950/70 to-slate-950/30"></div>
                    </div>
